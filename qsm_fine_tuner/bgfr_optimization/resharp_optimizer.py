...
def configure_experiment_run(test_fn, first_line="Optimization results: "):
    ...
    iter_folder = rf"E:\msc_data\sc_qsm\final_gauss_sims\feb_2026\bgfr_opt\snr_50\iter_resharp/{test_fn}"
    ...
    txt_file_path = rf"E:\msc_data\sc_qsm\final_gauss_sims\feb_2026\bgfr_opt\snr_50\iter_resharp/{test_fn}.txt"
...
def log_best_solution(obj_value, iteration, radius, alpha, gm_rmse, wm_rmse, wRMSE=None):
    global best_obj_value
    total_rmse = gm_rmse + wm_rmse
    if obj_value <= best_obj_value:
        out_text = f"#Iter {iteration}: OBJ {obj_value} // SMV radius: {radius}, Reg. Param aplha: {alpha}, GM RMSE: {gm_rmse}, WM RMSE: {wm_rmse}"
...
def resharp_optimizer(x):
    ...
    objective_value = gm_rmse + wm_rmse
    log_best_solution(objective_value, counter, radius, alpha, gm_rmse, wm_rmse, wRMSE=wRMSE)

    print(f"Iter {counter}: SMV Radius ={radius}, Regularization parameter: {alpha} GM+WM RMSE={objective_value}, wRMSE= {wRMSE}")
...
first_line = "BGFR optimization of RESHARP @SNR 50:"
configure_experiment_run("RMSE_test1_smv1_only_lambda")
