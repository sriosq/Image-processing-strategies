...
def configure_experiment_run(test_fn, first_line="Optimization results: "):
    ...
    iter_folder = rf"E:\msc_data\sc_qsm\final_gauss_sims\feb_2026\bgfr_opt\snr_50\iter_lbv/{test_fn}"
    ...
    txt_file_path = rf"E:\msc_data\sc_qsm\final_gauss_sims\feb_2026\bgfr_opt\snr_50\iter_lbv/{test_fn}.txt"
...
def log_best_solution(obj_value, iteration, tolerance, depth, peel, gm_rmse, wm_rmse, wRMSE=None):
    global best_obj_value
    total_rmse = gm_rmse + wm_rmse

    if obj_value <= best_obj_value:
        out_text = f"Iter #{iteration}: Tolerance: {tolerance}, Depth: {depth}, Peel: {peel} || GM RMSE: {gm_rmse}, WM RMSE: {wm_rmse} | RMSE: {total_rmse} \n"
...
def lbv_optimizer(x):
    ...
    objective_value = gm_rmse + wm_rmse
    log_best_solution(objective_value, counter, tolerance, depth, peel, gm_rmse, wm_rmse, wRMSE=wRMSE)

    print(f"Iter {counter}: Tolerance={tolerance}, Depth={depth}, Peel={peel}, GM+WM RMSE={objective_value}, wRMSE: {wRMSE}")
...
first_line = "BGFR optimization of LBV with fixed peel to 1 @SNR 50:"
configure_experiment_run("RMSE_test1_fixed_peel", first_line=first_line)
