...
def create_chimap(in1, in2, in3, in4 , output_basename, mask_filename, padisze):
    ...
    #  Star QSM optimizer uses only padsize as parameter
    
    padsize_param = np.ones(3) * padisze  # Convert to list for MATLAB compatibility
    ...
    "method": "Star-QSM",
    'padsize':padsize_param}

}

    eng.python_wrapper(in1, in2, in3, in4 , 'Star-QSM', output_basename, mask_filename, dipole_inv_params, nargout = 0)
...
def configure_experiment_run(test_fn):
    ...
    iter_folder = rf"E:\msc_data\sc_qsm\final_gauss_sims\July_2025\mrsim_outputs\custom_params\sus_mapping_opt\iter_StarQSM/{test_fn}"
    ...
    txt_file_path = rf"E:\msc_data\sc_qsm\final_gauss_sims\July_2025\mrsim_outputs\custom_params\sus_mapping_opt\iter_StarQSM/{test_fn}.txt"
...
def log_best_solution(obj_value, iteration, padSize, gm_rmse, wm_rmse):
    ...
    if obj_value <= best_obj_value:
        if obj_value == best_obj_value:
            ...
            with open(txt_file_path, 'a') as file:
                file.write(f"Iteration: {iteration}: OBJ {obj_value} // Pad size: {padSize}, GM RMSE: {gm_rmse}, WM RMSE: {wm_rmse} | RMSE: {total_rmse} \n")
...
def star_qsm_optimizer(x):
    ...
    iteration_fn = f"star_QSM_run{counter}/"
    ...
    create_chimap(in1, in2, in3, in4, output_fn, mask_filename, padSize)
    ...
    log_best_solution(objective_value, counter, padSize, gm_rmse, wm_rmse)

    print(f"Iter {counter}: Pad Size={padSize}, GM+WM RMSE = {objective_value}")
...
x0 = [12] # Recommended by SEPIA (for brain)
...
result = nomad.optimize(star_qsm_optimizer, x0, lb, ub, nomad_params)
